...
<div class="container-fluid">
    <div class="inner-body" wire:init="loadEmail">
        <div class="page-header">
            <div>
                <h2 class="main-content-title tx-24 mg-b-5">پیامک ها</h2><br>
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{route('Dashboard')}}">داشبورد</a></li>
                    <li class="breadcrumb-item active" aria-current="page">پیامک ها</li>
                </ol>

            </div>
            <div >
                <a class="btn btn-primary text-white my-2 btn-icon-text" href="{{route('AddSmsNotification')}}">افزودن
                    ...
                    <div class="card-header p-3 tx-medium my-auto tx-white bg-primary">
                        ...
                        @if(count($deleteItem) >=1 )
                            <span class="float-left">
                                    <a href="" wire:click.prevent="confirmAllEmailRemoval()" class="btn btn-sm btn-danger">
                                        حذف (
                                    {{count($deleteItem)}})
                                     </a>
                        </span>
                                    ...
                                    @if($readyToLoad)
                                        ...
                                        @foreach($smses as $sms)
                                            ...
                                            <tr role="row">
                                                <td>
                                                    <label class="ckbox" >
                                                        <input name="selected" value="{{$sms->id}}" wire:model="mulitiSelect" type="checkbox"><span class="tx-13"></span>
                                                    </label>
                                                </td>
                                                ...
                                                <td> {{$sms->id}} </td>
                                                <td>
                                                    {!! \Illuminate\Support\Str::limit($sms->content,30,'...') !!}
                                                </td>

                                                <td>{{ verta($sms->created_at)->format('%d/ %B  / %Y') }}</td>
                                                <td>
                                                    @if($this->expire($sms->date_send,$sms->time_send))
                                                        @if($sms->status == 1)
                                                            ارسال شده
                                                        @else
                                                           در حال ارسال
                                                        @endif
                                                    @else
                                                        {{$sms->date_send}}  -    {{$sms->time_send}}

                                                    @endif
                                                </td>
                                                <td>

                                                        <a href="{{route('editSmsNotification',$sms->id)}}" class="btn btn-sm btn-info">
                                                            <i class="fe fe-edit-2"></i>
                                                        </a>

                                                    <a href="" wire:click.prevent="confirmEmailRemoval({{ $sms->id }})" class="btn btn-sm btn-danger">
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <h4>برای حذف پیامک اطمینان دارید؟</h4>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button"  wire:click.prevent="delete" class="btn btn-danger"><i
                            class="fa fa-trash ml-1"></i>حذف پیامک
                    </button>
                </div>
            </div>
        </div>
    ...
    <div class="modal fade" id="form" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <h4>برای حذف گروهی پیامک ها اطمینان دارید؟</h4>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button"  wire:click.prevent="deleteAll()" class="btn btn-danger"><i
                            class="fa fa-trash ml-1"></i>حذف پیامک ها
                    </button>
                </div>
            </div>
        </div>
