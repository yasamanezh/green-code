...
<div class="container-fluid" style="overflow-x: hidden !important;">
    <div class="inner-body" wire:init="loadFilter">
        <div class="page-header">
            <div>
                <h2 class="main-content-title tx-24 mg-b-5">فیلترها</h2><br>
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{route('Dashboard')}}">داشبورد</a></li>
                    <li class="breadcrumb-item active" aria-current="page">فیلترها</li>
                </ol>
            </div>
            <div>
                <a href="{{route('AddFilter')}}" class="btn btn-primary">
                    افزودن
                                    ...
                                    <table class="table dataTable no-footer dtr-inline collapsed" id="example2"
                                        ...
                                        @if($readyToLoad)
                                            ...
                                            @foreach($data_info as $info)
                                                @php $attributes=\App\Models\Attribute::where('id',$info->attribute)->where('category_id',$info->category_id)->first();
                                                    $cat=\App\Models\Category::where('id',$info->category_id)->first();
                                                @endphp
                                                <tr role="row">
                                                    <td>
                                                        <label class="ckbox">
                                                    ...
                                                    <td>
                                                        {!! \Illuminate\Support\Str::limit($info->title,30,'...') !!}
                                                    </td>
                                                    <td>{{ verta($info->created_at)->format('%d / %B  / %Y') }}</td>

                                                    <td> @isset($attributes) {{$attributes->title}} @endisset </td>
                                                    <td>
                                                        @isset($cat)
                                                    ...
                                                    <td>{{$info->attribute_id}}</td>
                                                    <td>
                                                        @if($info->status==1)

                                                            <label class="custom-switch"
                                                                   wire:click.prefetch.prevent="statusDisable({{$info->id}})">
                                                                <input type="checkbox"
                                                        ...
                                                        @else
                                                            <label class="custom-switch"
                                                                   wire:click.prefetch.prevent="statusEnable({{$info->id}})">
                                                                <input type="checkbox"
                                                                       name="custom-switch-checkbox"
                                                                       class="custom-switch-input">
                                                                <span
                                                    ...
                                                    <td>
                                                        <a href="{{route('editFilter',$info->id)}}"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <h4>برای حذف گروهی فیلتر  ها اطمینان دارید؟</h4>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" wire:click.prevent="deleteAll()" class="btn btn-danger"><i
                            class="fa fa-trash ml-1"></i>حذف فیلتر  ها
                    </button>
                </div>
            </div>
        </div>
    ...
    <div class="modal fade" id="confirmationModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <h4>برای حذف فیلتر اطمینان دارید؟</h4>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" wire:click.prevent="delete" class="btn btn-danger"><i
                            class="fa fa-trash ml-1"></i>حذف فیلتر
                    </button>
                </div>
            </div>
        </div>
