...
class Comment extends Component
{
    ...
    public function addUseful($id)
    {
        $ip=$_SERVER['REMOTE_ADDR'];
        $usefullComment=commentUseful::where('comment_id',$id)->where('ip',$ip)->first();
        if($usefullComment){
            $usefullComment->no_useful=0;
            $usefullComment->useful=1;
            $usefullComment->update();

        }else{
            $newusefullComment=new commentUseful();
            $newusefullComment->ip=$ip;
            $newusefullComment->comment_id=$id;
            $newusefullComment->useful=1;
            $newusefullComment->no_useful=0;
            $newusefullComment->save();
    ...
    public function addUNoseful($id)
    {
        $ip=$_SERVER['REMOTE_ADDR'];
        $usefullComment=commentUseful::where('comment_id',$id)->where('ip',$ip)->first();
        if($usefullComment){
            $usefullComment->no_useful=1;
            $usefullComment->useful=0;
            $usefullComment->update();
            ...
            $newusefullComment->ip=$ip;
            $newusefullComment->comment_id=$id;
            $newusefullComment->no_useful=1;
            $newusefullComment->useful=0;
            $newusefullComment->save();
