...
class Trashed extends Component
{
    ...
    public function delete(){
        ...
        if(Gate::allows('delete_page')){
            $data_info_id=Demo::withTrashed()->findOrFail($this->categoryIdBeingRemoved);
    ...
    public function trashedCategory($id)
    {
        if(Gate::allows('edit_page')){
            $page = Demo::withTrashed()->where('id', $id)->first();
            $page->restore();
            Log::create([
                'user_id' => auth()->user()->id,
    ...
    public function render()
    {
        ...
        $data_info = DB::table('demos')
            ->whereNotNull('deleted_at')
            ->where('title','LIKE',"%{$this->search}%")
            ->latest()->paginate($this->count_data);

        return view('livewire.admin.demo.trashed',compact('data_info'));
    }
}
